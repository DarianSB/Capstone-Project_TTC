...
data2_filled_updated1 <- data2_filled_updated %>%
  mutate(Delay_Severity = case_when(
    `Min_Delay_Winsorized` >= 0 & `Min_Delay_Winsorized` < 10 ~ "Borderline Late",
    `Min_Delay_Winsorized` >= 10 & `Min_Delay_Winsorized` <= 15 ~ "Considerably Late",
    `Min_Delay_Winsorized` > 15 ~ "Extremely Late",
...
Delay_Severity_balance <- data2_filled_updated1 %>%
...
data2_filled_updated2 <- data2_filled_updated1 %>%
  mutate(Incident = recode(Incident,
                            "Cleaning" = "General Delay n' Emergencies",
                            "Collision" = "General Delay n' Emergencies",
                            "Diversion" = "General Delay n' Emergencies",
                            "Emergency Services" = "General Delay n' Emergencies",
                            "General Delay" = "General Delay n' Emergencies",
                            "Investigation" = "General Delay n' Emergencies",
                            "Management" = "General Delay n' Emergencies",
                            "Mechanical" = "Mechanical issue",
                            "Operations" = "Maintenance Operations",
                            "Overhead" = "General Delay n' Emergencies",
                            "Security" = "General Delay n' Emergencies",
                            "Vision" = "General Delay n' Emergencies"
                            ))
```
```{r}
# Create a customized boxplot to clearly see the interquartile range
...
# Count the frequency of each level in the 'Incident' column and calculate percentage
Delay_Severity_balance <- data2_filled_updated2 %>%
  count(Delay_Severity) %>%
  mutate(Percentage = n / sum(n) * 100) %>%
  arrange(desc(n))
...
Cleaned_Data$Time_Period<- as.factor(Cleaned_Data$Time_Period)
...
```{r}
normalized<-as.data.frame(lapply(Cleaned_Data[c(3,5,7,10,11)],normalize))
```
...
```{r}
#Forward Selection
full<-lm(data2_filled_updated2$Incident~.,data=Preprocessed_dataset)
null<-lm(data2_filled_updated2$Incident~1,data=Preprocessed_dataset)
