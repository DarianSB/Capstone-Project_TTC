...
```
```{r}
#Presence of complex and non-linear relationships among the dependent variable and independent variables; the relationship cannot be adequately captured or described by simple linear relationships.
# As 'Min_Gap'; 'Min_Delay' and 'Delay_Severity' are pretty similar variables generated from one another; they show strong correlations
```
...
full <- lm(Min_Delay_Winsorized ~ Incident + Time_Period + Route + Day + Direction + Vehicle , data = data2_filled_updated2)
...
```{r}
subsets<-regsubsets(Min_Delay_Winsorized ~ Incident + Time_Period + Route + Day + Direction + Vehicle , data = data2_filled_updated2, nbest=1)
sub.sum <- summary(subsets)
as.data.frame(sub.sum$outmat)
```

```{r}
#Outcome of Forward Selection and Backward Elimination exercise on TTC Delay data:
# All 6 features have some predictive power in varying lengths [AIC increase/decrease]
# Based on the output, we'll select 6 variables for predictive classification models which are: Incident, Route, Time Period, Day, Direction, Vehicle
```
...
#DOWNLOAD 'PREPROCESSED DATASET' TO BE USED SEPARATELY FOR MACHINE LEARNING MODELS
```{r}
library(shiny)
